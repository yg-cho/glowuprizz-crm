...
/**
 * 운영자 소유권 확인. 남의 리소스는 존재 여부를 숨기기 위해 항상 404.
...
export class OwnershipService {
  ...
  async campaign(operatorId: string, id: string) {
    const c = await this.prisma.campaign.findFirst({ where: { id, operatorId } });
  ...
  async form(operatorId: string, id: string) {
    const f = await this.prisma.form.findFirst({ where: { id, campaign: { operatorId } } });
    if (!f) throw new NotFoundException('form not found');
    return f;
  }

  async link(operatorId: string, id: string) {
    const l = await this.prisma.distributionLink.findFirst({ where: { id, form: { campaign: { operatorId } } } });
