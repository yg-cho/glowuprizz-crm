...
export function FormCompare({ rows }: { rows: FormStats[] }) {
    ...
    <Table>
      <TableHeader>
        <TableRow>
          ...
          {rows.length >= 2 && <TableHead className="text-right">차이</TableHead>}
        </TableRow>
      </TableHeader>
      <TableBody>
        <TableRow>
          ...
          {rows.length >= 2 && <TableCell />}
        </TableRow>
        {stageRows.map((s) => (
          <TableRow key={s.type}>
            <TableCell>{s.label}</TableCell>
            {rows.map((f) => <TableCell key={f.formId} className="text-right tabular-nums">{pct(f.stages[s.idx].stepRate)}</TableCell>)}
            {rows.length >= 2 && <TableCell className={cn('text-right tabular-nums text-muted-foreground')}>{diff(rows[1].stages[s.idx].stepRate, base.stages[s.idx].stepRate)}</TableCell>}
          </TableRow>
        ))}
        <TableRow>
          ...
          {rows.length >= 2 && <TableCell className="text-right font-medium tabular-nums">{diff(rows[1].overallRate, base.overallRate)}</TableCell>}
        </TableRow>
      </TableBody>
    </Table>
  );
