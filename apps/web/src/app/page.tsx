...
function Dashboard() {
  ...
  const { data: series } = useApi<TimeseriesPoint[]>(`/stats/timeseries?${q}`);
  const { data: channels } = useApi<ChannelStats[]>(`/stats/channels?${q}`);
  const { data: campaigns } = useApi<CampaignStats[]>(`/stats/campaigns?${q}`);
