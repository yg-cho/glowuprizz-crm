...
export interface CampaignDetailInitial {
...
export function CampaignDetailView({ id, initial }: { id: string; initial: CampaignDetailInitial }) {
  const router = useRouter();
  const { filters, set, query } = useStatsFilters({ range: '7d', compare: true });
  const q = query({ campaignId: id });

  const { data: campaign } = useApi<Campaign>(`/campaigns/${id}`, initial.campaign);
  const { data: templates } = useApi<Template[]>('/templates', initial.templates);
  const funnelQ = useApi<Funnel>(`/stats/funnel?${q}`, initial.funnel);
  const linksQ = useApi<LinkStats[]>(`/stats/links?${q}`, initial.links);
  const formsQ = useApi<FormStats[]>(`/stats/forms?${q}`, initial.forms);
  const heatmapQ = useApi<HeatmapData>(`/stats/heatmap?${q}`, initial.heatmap);
  const failuresQ = useApi<Failure[]>(`/stats/failures?${q}`, initial.failures);
  const qualityQ = useApi<Quality>(`/stats/quality?${q}`, initial.quality);
  const publicOrigin = usePublicOrigin(initial.publicOrigin);
