...
export function DashboardView({ initial }: { initial: DashboardInitial }) {
  ...
  const q = query();
  const { data: campaignList } = useApi<Campaign[]>('/campaigns', initial.campaignList);
  const funnelQ = useApi<Funnel>(`/stats/funnel?${q}`, initial.funnel);
  const seriesQ = useApi<TimeseriesPoint[]>(`/stats/timeseries?${q}`, initial.series);
  const channelsQ = useApi<ChannelStats[]>(`/stats/channels?${q}`, initial.channels);
  const campaignsQ = useApi<CampaignStats[]>(`/stats/campaigns?${q}`, initial.campaigns);
  const { data: failures } = useApi<Failure[]>(`/stats/failures?${q}`, initial.failures);
  const { data: insights } = useApi<Insight[]>(`/stats/insights?${q}`, initial.insights);
