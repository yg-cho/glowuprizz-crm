...
export interface FormDetailInitial {
  form: FormRow | null;
  links: LinkStats[] | null;
  submissions: SubmissionPage | null;
  publicOrigin: string | null;
}

export function FormDetailView({ id, initial }: { id: string; initial: FormDetailInitial }) {
  const router = useRouter();
  const form = useApi<FormRow>(`/forms/${id}`, initial.form);
  const links = useApi<LinkStats[]>(`/stats/links?range=all&formId=${id}`, initial.links);
    ...
    <>
      ...
      <div className="grid gap-3 lg:grid-cols-3">
        ...
        <Section title="신청 명단" desc="이 폼으로 들어온 신청. 행을 펼치면 방문자 여정." className="lg:col-span-3">
          <SubmittedList formId={id} compact initial={initial.submissions} />
        </Section>
      </div>
    </>
  );
