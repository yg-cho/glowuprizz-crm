...
| [0007](0007-funnel-stages-and-events.md) | 퍼널 단계 정의(5단계+실패)와 이벤트 수집·집계 | 채택 |

## 요구사항 해석 시 세운 가정 (요약)

| 불분명한 요구 | 가정 | 근거 ADR |
|---|---|---|
| "방문" / "방문자" | 방문 = 페이지뷰(VIEW 이벤트), 방문자 = 익명 쿠키 기준 고유 수 | 0005, 0007 |
| 퍼널 단계 | 링크 클릭 → 폼 도달 → 작성 시작 → 제출 시도 → 신청 완료 (+실패). 단계 수는 고유 방문자 | 0007 |
| 봇 | 링크 클릭은 있으나 폼 도달(스크립트 실행) 없는 방문자로 추정 | 0007 |
| "전환율" | 신청 ÷ 방문자 | 0005 |
| "채널별 성과" | 채널 배포 링크로 들어온 이벤트만(마지막 클릭 귀속). 직접 접근은 제외 | 0005, 0007 |
